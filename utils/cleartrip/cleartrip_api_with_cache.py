#!/usr/bin/env python3
"""
Cleartrip API with Cookie Caching and Auto-Refresh
Saves cookies to file and refreshes them automatically when expired
"""
...
class CleartripAPIWithCache:
    def __init__(self, cookie_file='cleartrip_cookies.pkl'):
        self.session = requests.Session()
        self.cookie_file = cookie_file
        self.base_url = "https://www.cleartrip.com"
    ...
    def load_cookies(self):
        ...
        try:
            ...
            # Check if cookies are less than 12 hours old
            cookie_time = datetime.fromisoformat(data['timestamp'])
            if datetime.now() - cookie_time < timedelta(hours=12):
                print(f"✓ Loaded cached cookies (age: {datetime.now() - cookie_time})")
                return data['cookies']
            else:
                print("Cached cookies are too old, need refresh")
                return None
    ...
    def set_cookies_from_string(self, cookie_string):
        ...
        for pair in cookie_pairs:
            if "=" in pair:
                ...
                self.session.cookies.set(key, value, domain=".cleartrip.com")
                cookies_dict[key] = value
        
        self.save_cookies(cookies_dict)
        print(f"✓ Set {len(cookies_dict)} cookies")
        return cookies_dict
    
    def set_cookies_from_dict(self, cookies_dict):
        """Set cookies from dictionary"""
        for key, value in cookies_dict.items():
            self.session.cookies.set(key, value, domain=".cleartrip.com")
    
    def create_itinerary(self, payload, auto_retry=True):
        """
        Make the POST request to create itinerary
        If it fails with 500 error, prompt for new cookies
        """
        url = f"{self.base_url}/itin/v7/itinerary/create"
        
        # Try to load cached cookies
        cached_cookies = self.load_cookies()
        if cached_cookies:
            self.set_cookies_from_dict(cached_cookies)
        ...
        try:
            print("\nMaking POST request to Cleartrip API...")
            ...
            elif response.status_code == 500 and auto_retry:
                print("\n⚠ Got 500 error - cookies may be expired")
                print("Please provide fresh cookie string from browser:")
                print("(Open browser DevTools > Network > Copy cookie header)")
                cookie_string = input("\nPaste cookie string here: ").strip()
                
                if cookie_string:
                    self.set_cookies_from_string(cookie_string)
                    print("\nRetrying request with new cookies...")
                    return self.create_itinerary(payload, auto_retry=False)
...
def main():
    ...
    response_data = api.create_itinerary(payload)
